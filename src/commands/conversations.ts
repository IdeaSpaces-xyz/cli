...
/**
 * Build the `conversations` (list) command. `local.list` handles `--local`; the
 * router injects the Pi implementation so this core command stays Pi-free.
 */
export function makeConversationsCommand(local: LocalConversationOps): CommandDef {
  return {
    name: "conversations",
    ...
    async run(args, flags, global) {
      ...
      if (flags.local) return local.list(flags, output);
